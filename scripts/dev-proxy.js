...
function startShopify(server) {
  ...
  // Use shell so environment assignment works cross-platform in one command
  const cmd = 'shopify app dev';
...
const check = require('child_process').spawnSync('lsof', ['-i', `:${PROXY_PORT}`, '-sTCP:LISTEN', '-Pn']);
